...
export async function generateSiteConfigWithGemini(input) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            if (usageMetadata) {
                ...
                const estimatedCost = (inputTokens / 1_000_000) * GEMINI_PRICING.input +
            ...
            if (Array.isArray(sec.instructors)) {
                sec.instructors = sec.instructors.map((inst) => ({
            ...
            if (Array.isArray(sec.objections)) {
                sec.objections = sec.objections.map((o) => ({
            ...
            if (Array.isArray(sec.requests)) {
                sec.requests = sec.requests.map((r) => {
                    if (typeof r === 'string') {
                        return { image: '', text: r };
                    }
            ...
            if (Array.isArray(sec.atmosphere)) {
                sec.atmosphere = sec.atmosphere.map((a) => {
                    if (typeof a === 'string') {
            ...
            if (Array.isArray(sec.advantages)) {
                sec.advantages = sec.advantages.map((a) => ({
                    title: a.title || '',
            ...
            if (sec.quiz && Array.isArray(sec.quiz.steps)) {
                sec.quiz.steps = sec.quiz.steps.map((step) => ({
            ...
            if (Array.isArray(sec.calculatorStages)) {
                sec.calculatorStages = sec.calculatorStages.map((s) => ({
            ...
            if (Array.isArray(sec.directions)) {
                sec.directions = sec.directions.map((d, i) => ({
                    id: d.id || `dir-${i}`,
                    title: d.title || '',
                    image: d.image || '',
