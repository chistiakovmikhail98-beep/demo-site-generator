...
async function test1_HealthCheck() {
    ...
    if (!data.total || data.requests === undefined)
...
async function test2_PreviewAPI() {
    ...
    const data = await response.json();
    if (!data.fullConfig)
        throw new Error('No fullConfig in response');
    if (!data.fullConfig.brand?.name)
        throw new Error('No brand.name in fullConfig');
...
async function test4_QuickWithFullConfig() {
    ...
    if (!previewData.fullConfig)
        throw new Error('No fullConfig from preview');
