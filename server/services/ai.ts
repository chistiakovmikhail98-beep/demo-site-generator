...
const PRICING_MAP: Record<AIModel, { input: number; output: number }> = {
...
export interface TokenUsage {
  ...
  totalTokens: number;
  estimatedCost: number; // в долларах
}
...
export const tokenStats = {
  ...
  requests: 0,
  byProject: new Map<string, TokenUsage>(),
};
...
interface GenerateInput {
  ...
  niche: Niche;
  description?: string;
  imageFiles: UploadedFile[];  // Фото с metadata (block, label)
  textContent?: string;
...
// Парсинг сырого текста — AI извлекает название и нишу
export interface ParsedStudioInfo {
  name: string;
  niche: Niche;
  description: string;
}

export async function parseRawText(rawText: string, aiModel: AIModel | string = DEFAULT_MODEL): Promise<ParsedStudioInfo> {
  const openai = getOpenAI();

  // Определяем модель
  const modelKey = (aiModel in MODEL_MAP ? aiModel : DEFAULT_MODEL) as AIModel;
  const modelId = MODEL_MAP[modelKey];
  currentPricing = PRICING_MAP[modelKey];
