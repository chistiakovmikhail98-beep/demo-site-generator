...
async function updateIndexHtml(buildDir: string, config: SiteConfig): Promise<void> {
  ...
  html = html.replace(
    ...
    `<meta name="description" content="${esc(config.brand.tagline)}">`
  );
...
function getDefaultCalculatorStages(niche: string): Array<{status: string; description: string; tags: string[]; achievement: string}> {
...
function getDefaultSectionTitles(niche: string, directionsCount: number) {
...
function getExpertiseByNiche(niche: string): string {
  const expertiseByNiche: Record<string, string> = {
...
async function updateViteConfig(buildDir: string): Promise<void> {
 ...
 */
function applyUploadedPhotos(config: SiteConfig, files: UploadedFile[]): SiteConfig {
  // Группируем файлы по типу блока
  const byBlock: Record<PhotoBlockType, UploadedFile[]> = {
    hero: [],
    directions: [],
    instructors: [],
    stories: [],
  ...
  for (const block of Object.keys(byBlock) as PhotoBlockType[]) {
