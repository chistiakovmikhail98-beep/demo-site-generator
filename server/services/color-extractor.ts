...
export interface ColorScheme {
  primary: string;   // Основной цвет (для кнопок, заголовков)
  accent: string;    // Акцентный цвет (для hover, второстепенных элементов)
  background?: string; // Фон (опционально)
}
 ...
 */
export async function extractColorsFromBuffer(imageBuffer: Buffer): Promise<ColorScheme> {
  ...
  if (contrastColor) {
    const accentRgb = hslToRgb(
      contrastColor.h,
      Math.max(contrastColor.s, 50),
      Math.min(Math.max(contrastColor.l, 50), 70)
    );
    ...
    const accentRgb = hslToRgb(
      primaryColor.h,
      Math.max(primaryColor.s - 10, 40),
      Math.min(primaryColor.l + 20, 75)
    );
    accentHex = rgbToHex(accentRgb.r, accentRgb.g, accentRgb.b);
  }
