...
async function ensureSSHKey(): Promise<string | null> {
  if (sshKeyPath) return sshKeyPath;
  if (!VPS_SSH_KEY_CONTENT) return null;
 ...
 * Удаляет сайт с VPS
 */
export async function deleteFromVPS(slug: string): Promise<boolean> {
 ...
 */
export async function listVPSSites(): Promise<string[]> {
  if (!VPS_HOST) return [];
