...
import { QUIZ_CONFIG, BRAND_CONFIG } from '../constants';

// Используем данные из constants
const MANAGER_AVATAR = QUIZ_CONFIG.managerImage;
const MANAGER_NAME = QUIZ_CONFIG.managerName;

// Генерируем MANAGER_TIPS из массива tips
const MANAGER_TIPS: Record<number, string> = {};
QUIZ_CONFIG.tips.forEach((tip, idx) => {
  MANAGER_TIPS[idx] = tip;
});

// Формируем шаги квиза из QUIZ_CONFIG.steps + добавляем contact и final
const generateSteps = () => {
  const quizSteps = QUIZ_CONFIG.steps.map((step, idx) => ({
    id: idx + 1,
    key: `step_${idx}`,
    title: step.question,
  ...
  // Вставляем контактный шаг после 3-го вопроса (или в конец если вопросов меньше)
                    ...
                    <div className="bg-[#121215] border border-zinc-800 rounded-[2rem] md:rounded-[2.5rem] p-5 md:p-12 shadow-2xl relative overflow-hidden mb-6">
                        ...
                        <div className="relative z-10">
                            <div className="text-center mb-6 md:mb-10">
                                ...
                                <p className="text-zinc-500 text-sm md:text-base">Персональные рекомендации от {BRAND_CONFIG.name}</p>
                            </div>
