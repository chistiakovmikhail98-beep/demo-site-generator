  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: 'welcome',
      sender: 'bot',
      text: AI_CHAT_CONFIG.welcomeMessage
    }
  ]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const historyString = messages
        .slice(-6)
        .map(m => `${m.sender === 'user' ? 'Пользователь' : `${AI_CHAT_CONFIG.managerName} (AI)`}: ${m.text}`)
        .join('\n');

      // 4. Формируем строку с ценами из PRICING
      const pricingInfo = PRICING.slice(0, 4).map(p => `${p.name}: ${p.price}`).join(', ');

      // 5. Собираем полный промпт для AI
      const fullPrompt = `
...
ТВОЯ ЗАДАЧА:
Ответь пользователю от имени ${AI_CHAT_CONFIG.managerName} (${AI_CHAT_CONFIG.managerTitle} ${BRAND_CONFIG.name}${BRAND_CONFIG.city ? ` в ${BRAND_CONFIG.city}` : ''}).
Ниша: ${BRAND_CONFIG.niche}.
Будь профессиональной, эмпатичной и поддерживающей.
Если спрашивают цены: ${pricingInfo}.
${CONTACTS.phone ? `Телефон для записи: ${CONTACTS.phone}.` : ''}
      ...
      <div className={containerClasses}>
        {!isWidget ? (
            <div className="bg-zinc-900/50 p-4 md:p-8 md:w-1/3 flex flex-col md:justify-between border-b md:border-b-0 md:border-r border-zinc-800">
                <div>
                    <div className="flex items-center gap-3 md:gap-4 mb-2 md:mb-6">
                        ...
                        <div>
                            <h3 className="text-white font-bold text-base md:text-lg">{AI_CHAT_CONFIG.managerName.split(' ')[0]}</h3>
                            <p className="text-primary text-[10px] md:text-xs font-bold uppercase tracking-wider">{AI_CHAT_CONFIG.managerTitle}</p>
                        </div>
                    </div>
                    <div className="hidden md:block">
                        <p className="text-zinc-400 text-sm leading-relaxed mb-4">
                            На связи и готов(а) помочь вам с любыми вопросами о {BRAND_CONFIG.name}.
                        </p>
                    </div>
                </div>
            ...
            <div className="bg-primary p-4 flex items-center gap-3 border-b border-white/10">
                ...
                <div>
                    <h3 className="text-white font-bold text-sm leading-none uppercase tracking-tight">{AI_CHAT_CONFIG.managerName}</h3>
                    <p className="text-red-200 text-[10px] font-medium mt-0.5">{AI_CHAT_CONFIG.managerExpertise}</p>
                </div>
            </div>
        )}
            ...
            <div className="p-3 bg-zinc-900/50 border-t border-zinc-800">
                <div className="relative flex items-center gap-2">
                    <input
                        ...
                        placeholder={AI_CHAT_CONFIG.placeholderText}
