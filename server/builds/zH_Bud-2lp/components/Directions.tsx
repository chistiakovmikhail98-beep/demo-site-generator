...
import { DIRECTIONS, SECTION_TITLES, DIRECTIONS_TABS } from '../constants';
...
const Directions: React.FC = () => {
  ...
  // Используем динамические табы из констант или fallback
  const tabs = DIRECTIONS_TABS || [
    { key: 'all', label: 'Все' },
  ...
  // Фильтруем табы — показываем только те, у которых есть направления
  const visibleTabs = useMemo(() => {
    return tabs.filter(tab => {
      if (tab.key === 'all') return true;
      return DIRECTIONS.some(dir => dir.category === tab.category);
    });
  }, []);
  ...
  const filteredDirections = DIRECTIONS.filter(dir => {
    ...
    return dir.category === activeTab?.category;
  });

  const displayedDirections = showAll ? filteredDirections : filteredDirections.slice(0, 6);

  // Динамический заголовок с количеством направлений
  const titleText = SECTION_TITLES?.directions?.title?.replace('{count}', String(DIRECTIONS.length)) || `${DIRECTIONS.length} направлений`;
  const subtitleText = SECTION_TITLES?.directions?.subtitle || 'в одной студии';
    ...
    <Section id="directions" className="bg-zinc-100">
      <div className="mb-12">
        <h2 className="text-3xl md:text-6xl font-black text-zinc-900 mb-8 uppercase tracking-tight leading-[0.9]">
          {titleText} <br />
          <span className="text-zinc-400">{subtitleText}</span>
        </h2>
