...
export default function RequestsV1({ data, editable, onDataChange, onCTAClick }: BlockProps<RequestsData>) {
  ...
  return (
    <section className="relative bg-[#0c0c0e] py-12 sm:py-16 md:py-24 overflow-hidden">
      <style>{`
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-8 sm:mb-12 md:mb-16 text-center">
        ...
        {data.subtitle && (
          <EditableText
            ...
            editable={editable}
            as="p"
            className="mt-3 sm:mt-4 text-sm sm:text-base text-zinc-400 max-w-xl mx-auto"
          />
        )}
      </div>
          ...
          <div key={rowIdx} className="flex overflow-hidden py-1 sm:py-2">
            <div className={`flex gap-3 sm:gap-4 md:gap-6 px-1.5 sm:px-3 w-max ${rowIdx === 1 ? 'requests-v1-scroll-reverse' : 'requests-v1-scroll'}`}>
              ...
              {[...row, ...row, ...row].map((item, idx) => (
                <div
                  key={`${item.text}-${idx}`}
                  className="flex items-center gap-2 sm:gap-3 bg-white rounded-full py-3 px-4 sm:px-5 shadow-lg hover:scale-105 transition-transform duration-300 shrink-0"
                >
                  {isPlaceholder(item.image) ? (
                    <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-gradient-to-br from-primary/20 to-primary/40 border-2 border-zinc-100 shrink-0 flex items-center justify-center">
                    ...
                    <img
                      src={item.image}
                      alt=""
                      className="w-8 h-8 sm:w-10 sm:h-10 rounded-full object-cover border-2 border-zinc-100 shrink-0"
                      loading="lazy"
                    />
                  )}
                  <span className="text-zinc-900 font-bold whitespace-nowrap text-xs sm:text-sm">
                    {item.text}
                  </span>
                </div>
              ))}
